...
Null hypothesis testing can tell us whether an effect is statistically significant; however, it is often of interest to be able to say something about the size of the effect. For example, this might help  determine whether an effect is big enough to be practically or clinically significant. Effect sizes help facilitate these judgements by providing a measure of effect that can be compared across different studies, regardless of the units used (e.g., whether a depression study uses a measure like the PHQ-9 or the Beck Depression Inventory) or what the treatment is (e.g., cognitive behavioural therapy or antidepressant medications).  

DigiCAT offers an effect size measure for binary treatments and continuous outcomes. Specifically, Hedge’s g, which takes the difference between the treated and control group and divides it by a pooled standard deviation that is weighted by the relative sample sizes of the groups.  

It is calculated using the formula: 

$$
g = \frac{M_T - M_C}{\sqrt{\frac{SD_T^2 (n_T - 1) + SD_C^2 (n_C - 1)}{n_T + n_C - 2}}}
$$
Where M refers to the mean, SD refers to the standard deviation and n refers to the sample size of the treatment group (T) and control group (C).  

In the case of marginal effects (used when treatment by matching variable interactions are included in the model), the marginal treatment effect is used as the numerator. In the case of multiple imputation, the pooled mean difference and pooled standard deviations across the imputed datasets are used in the formula. In the case of complex survey designs, the relevant terms in the formula are weighted by the relevant design or attrition weight.  

You can read more about Hedge’s g in [this paper](https://journals.sagepub.com/doi/10.3102/10769986006002107).
...
In DigiCAT, we offer Fong et al.'s (2018) extension of the CBPS method proposed originally for binary treatments by Imai & Ratkovic (2014). This means it can be used for both binary and  continuous treatments. This method is referred to as the ‘covariate balancing generalised propensity score’. It uses a ‘method of moments’ methodology to estimate propensity scores that minimises the correlation between treatment and balancing variables, which is equivalent to saying that it estimates propensity scores that optimise the balance in covariates across those with higher and lower levels of the treatment variable.These propensity scores are then used to derive weights which are used in weighted regression of the outcome variable on the treatment variable. In this way, the method is similar to IPTW except that balance is explicitly optimised as part of the propensity score model. . Balance checking can be conducted by examining the weighted correlation between the treatment variable and each balancing variable  (Austin, 2019). It has been suggested that the amount of measured confounding can be considered  small when the average absolute correlation coefficient for each balancing variable is less than 0.1. 
